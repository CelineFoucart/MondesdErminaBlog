<x-admin-layout>
    <x-slot name="pageTitle">Catégorie</x-slot>
    <x-slot name="headerAdmin">Liste des catégories</x-slot>
    <x-slot name="header">
        <x-breadcrumb :links="[['route' => null, 'title' => 'Catégories'] ]"></x-breadcrumb>
    </x-slot>
    ...
    <div class="border rounded p-4 bg-white">
        ...
        <div class="text-right">
            <a href="{{ route('admin.category.create') }}" class=" mb-3 inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                Ajouter
            </a>
        </div>
            ...
            <body>
                @forelse ($categories as $category)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        <td class="px-6 py-4">
                            <a href="{{ route('blog.show', $category) }}">
                                {{ $category->title }}
                            </a>
                        </td>
                        ...
                        <td class="px-6 py-4">  
                            <a href="{{ route('admin.category.show', $category) }}" class="mt-4 text-white hover:text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 mr-1 rounded-lg text-sm px-3 py-2 dark:bg-green-600 dark:hover:bg-green-700 focus:outline-none dark:focus:ring-green-800">
